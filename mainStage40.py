    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                if epoch<=40:
                    loss_te_list = []
                    for ind in range(num_text_aug): # 从0到4，即0的整体+4个分组
                    # for ind in range(1, num_text_aug):  # 从 1 开始 只用pasta
                    # for ind in [0]:  # 只跑 synonym
                        if ind > 0:
                    ...
                    all_text_features = []  # 存储所有文本特征
                    all_skeleton_features = []  # 所有骨骼特征
                    all_scale = []

                    for ind in range(1, num_text_aug):  # 从0到4，即0的整体+4个分组
                        # for ind in range(1, num_text_aug):  # 从 1 开始 只用pasta
                        # for ind in [0]:  # 只跑 synonym
                        if ind > 0:
                            text_id = np.ones(len(label), dtype=np.int8) * ind
                            ...
                            all_skeleton_features.append(part_feature_list[ind - 1])

                        text_embedding = self.model_text_dict[self.arg.model_args['head'][0]](texts).float()
                        all_text_features.append(text_embedding)

                        all_scale.append(logit_scale[:, ind].mean())

                    fused_text_feature = torch.mean(torch.stack(all_text_features), dim=0)
                    fused_skeleton_feature = torch.mean(torch.stack(all_skeleton_features), dim=0)

                    fuse_scale = torch.mean(torch.stack(all_scale), dim=0)

                    logits_part_image, logits_part_text = create_logits(
                        fused_skeleton_feature,  # 使用整体特征
                        fused_text_feature,
                        fuse_scale
                    )
                    ground_truth = torch.tensor(label_g, dtype=feature_dict[self.arg.model_args['head'][0]].dtype,
                    ...
                    loss_part_imgs = self.loss(logits_part_image, ground_truth)
                    loss_part_texts = self.loss(logits_part_text, ground_truth)
                    loss_part_fuse = (loss_part_imgs + loss_part_texts) / 2
                    ...
                    texts = torch.cat(texts).cuda(self.output_device)
                    text_embedding = self.model_text_dict[self.arg.model_args['head'][0]](texts).float()
                    logits_image, logits_text = create_logits(feature_dict[self.arg.model_args['head'][0]],
                                                              text_embedding, logit_scale[:, 0].mean())

                    ground_truth = torch.tensor(label_g, dtype=feature_dict[self.arg.model_args['head'][0]].dtype,
                                                device=device)
                    loss_imgs = self.loss(logits_image, ground_truth)
                    loss_texts = self.loss(logits_text, ground_truth)
                    loss_whole = (loss_imgs + loss_texts) / 2

                    loss_ce = self.loss_ce(output, label)

                    loss = loss_ce + self.arg.loss_alpha * (loss_part_fuse + loss_whole) / 2
