...
class MHSA(nn.Module):
    def __init__(self, dim_in, dim, A, num_heads=6, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., insert_cls_layer=0, pe=False, num_point=25,
        ...
        self.pe = nn.Parameter(torch.zeros((num_v,num_v,num_heads,dim//num_heads)))
    ...
    def forward(self, x):
        ...
        # print('q,pe', q.shape, self.pe.shape)
        b = torch.einsum("bthnc, nmhc->bthnm", q, self.pe)


        # print('--------------------------------')
        # b*T分组数*V分组数,T维度,num_head,V维度,C
        a = q @ k.transpose(-2, -1)
...
class unit_vit(nn.Module):
    def __init__(self, dim_in, dim, A, num_of_heads, add_skip_connection=True,  qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.partition_size = [[8, 5], [4, 5], [8, 10], [4, 10]]
    ...
    def forward(self, x, joint_label, groups):
        ...
        split_x=torch.chunk(x_in,chunks=4,dim=1)
        # split_e=torch.chunk(e,chunks=4,dim=1)
...
class Model_lst_4part(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(N, M, V, C, T).contiguous().permute(0, 3, 4, 1, 2).contiguous().view(N, C, T, M * V)
        ...
        # N*M, C, T, V
        C_out=x.size(1)
        x=x.view(N,C_out,T//4,V,M)
        x=x.permute(0,4,1,2,3).contiguous().view(N*M,C_out,T//4,V)
